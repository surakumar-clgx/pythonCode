...
async def delete_files(folder):
    with ThreadPoolExecutor(max_workers=500) as executor:
        loop = asyncio.get_running_loop()
        tasks = [
            loop.run_in_executor(executor, os.remove, file) for file in folder.iterdir()
        ]
...
async def main(folders):
    tasks = [delete_folder(folder) for folder in folders]
    await asyncio.gather(*tasks)
...
#     # folder
#     # for folder in Path(
#     # r"C:\Users\surakumar\OneDrive - CoreLogic Solutions, LLC\Downloads\co_data\\"
#     # ).iterdir()
#     # if "_" in folder.name and folder.is_dir()
#     # ]
#     print(f"\n\n\nTotal number of folders to be deleted -: {len(mani)}\n\n")
